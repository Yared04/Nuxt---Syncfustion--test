<template>
  ...
  >
    <div class="mt-4 flex max-md:justify-center">
      <span class="relative flex h-10 mx-1 max-md:w-3/4">
        <!-- <i
    ...
    <ejs-treeview
      id="treeview"
      @contextmenu="disableDefaultRightClick"
      :fields="treeFields"
    ...
    >
      <template v-slot:nodeTemplate="{ data }">
        <font-awesome-icon
          v-if="data.nodeChild && data.nodeChild.length > 0"
          class="mr-2"
          :icon="['far', 'folder']"
        />
...
// };
const disableDefaultRightClick = (event) => {
  if (event.button === 2) {
...
//     // Add new item
//     var nodeId = "tree_" + index;
//     var newItem = { id: nodeId, name: "New Folder" };

//     // Update TreeView UI
//     treevalidate.addNodes([newItem], targetNodeId, null);
//     index++;
//     treevalidate.fields.dataSource.push(newItem);
//     treevalidate.beginEdit(nodeId);

//     // Call handleAddItems to update actual data
//     props.handleAddItems([{ name: newItem.name }]);
//   } else if (args.item.text === "Remove Item") {
//     // Remove item from TreeView UI
//     treevalidate.removeNodes([targetNodeId]);

//     // Call handleDelete to update actual data
//     props.handleDelete(targetNodeId);
//   } else if (args.item.text === "Rename Item") {
//     treevalidate.beginEdit(targetNodeId);

//     // When rename completes, call handleEditItem to update actual data
//     treevalidate.nodeEditing = (event) => {
//       const newName = event.value;
//       props.handleEditItem({ ...targetNode, title: newName });
//     };
//   }
// };
...
const beforeopen = () => {
  ...
  // Enable or disable "Remove Item" based on node class
  contentmenutree.enableItems(
    ["Remove Item"],
    !targetNode.classList.contains("remove")
  );
  contentmenutree.enableItems(
    ["Rename Item"],
    !targetNode.classList.contains("rename")
  );
};
